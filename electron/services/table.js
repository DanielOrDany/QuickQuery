...
async function loadTableResult(connectionName, alias, loadingOptions) {
    try {
        ...
        if (connection.dtype === FIRESTORE) {
            ...
            query += ` LIMIT ${limit}`; // without offset

            console.log("Query:", query);

            const rows = await fireSQL.query(query);
            ...
            let fields = [];
            if (rows.length > 0) {
                fields = Object.keys(rows[0]).map(field => {
                    return {
                        name: field
                    };
                });
            ...
            console.log("rows", queryResult[1].rows);
            console.log("fields", queryResult[1].fields);
...
async function getTableSize(connectionName, alias) {
    try {
        ...
        if (connection.dtype === FIRESTORE) {
            ...
            const result = await firebase_db.collection(table).get();
